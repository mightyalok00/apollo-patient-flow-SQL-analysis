...
export const BottleneckAnalysis: React.FC<BottleneckAnalysisProps> = ({ onSelectQuery }) => {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-gradient-to-r from-rose-950 via-slate-900 to-amber-950 text-white rounded-2xl p-6 sm:p-8 shadow-md">
        <div className="flex items-center space-x-2 text-xs font-semibold uppercase tracking-wider text-rose-300 mb-2">
          <ShieldAlert className="w-4 h-4" />
          <span>Question 14: Multi-Factor Bottleneck Scoring Model</span>
        </div>
        ...
        <p className="text-slate-300 text-sm leading-relaxed max-w-3xl mb-4">
          Combines statistical percentiles (<code className="font-mono bg-slate-800 px-1 py-0.5 rounded text-rose-300">PERCENT_RANK()</code>) across four operational dimensions: 
          <strong> Waiting Time</strong>, <strong> Length of Stay (LOS)</strong>, <strong> Readmission Rate</strong>, and <strong> Bed Utilization</strong> into a unified 0–100 pressure index.
        </p>

        <div className="flex flex-wrap gap-4 text-xs">
          <div className="bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700">
            <span className="text-slate-400">#1 Bottleneck: </span>
            <span className="font-bold text-rose-400">Apollo Delhi Emergency (92.11)</span>
          </div>
          <div className="bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700">
            <span className="text-slate-400">Longest Wait: </span>
            <span className="font-bold text-amber-400">Apollo Bangalore Emergency (106.71 min)</span>
          </div>
          <div className="bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700">
            <span className="text-slate-400">Peak Readmission: </span>
            <span className="font-bold text-orange-400">Apollo Delhi Orthopedics (40.77%)</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-rose-50 border border-rose-200 rounded-2xl p-5 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
          <div className="flex items-center space-x-2 text-rose-900 font-bold">
            ...
            <h2 className="text-base font-bold">Emergency Departments Account for All Top 4 Critical Bottlenecks</h2>
          </div>
          <button
            id="btn-inspect-q14-direct"
            onClick={() => onSelectQuery(14)}
            className="text-xs font-bold text-rose-800 hover:text-rose-950 bg-rose-200/80 hover:bg-rose-200 px-3 py-1.5 rounded-lg flex items-center space-x-1"
          >
            <span>Run SQL Query (Q14)</span>
            <ArrowUpRight className="w-3.5 h-3.5" />
          </button>
        </div>
        <p className="text-xs text-rose-800 leading-relaxed mb-4">
          All four hospital Emergency wards demonstrate average wait times exceeding 99 minutes (+65% above overall average), elevated 30-day readmissions (34.7% to 37.7%), and higher average length of stay (4.5 to 4.8 days).
        </p>
          ...
          {DEPARTMENT_BOTTLENECKS.filter(d => d.department_name === 'Emergency').map((em) => (
            <div key={em.hospital_name} className="bg-white p-3.5 rounded-xl border border-rose-200 shadow-xs">
              <div className="flex items-center justify-between text-xs font-bold text-slate-800 mb-1">
                <span>{em.hospital_name}</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-rose-100 text-rose-800">
                  Rank #{em.rank}
                </span>
              </div>
              <div className="text-xl font-black text-rose-600 mb-1">{em.bottleneck_score} <span className="text-xs text-slate-400 font-normal">/100</span></div>
              <div className="text-[11px] text-slate-600 space-y-0.5">
                <div>Wait: <span className="font-semibold text-slate-900">{em.avg_wait_minutes} min</span></div>
                <div>Stay: <span className="font-semibold text-slate-900">{em.avg_los_days} days</span></div>
                <div>Readm: <span className="font-semibold text-slate-900">{em.readmission_rate_pct}%</span></div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="lg:col-span-7 bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-2">
              <div>
                <h3 className="font-bold text-base text-slate-900">Length of Stay vs Readmission Matrix</h3>
                <p className="text-xs text-slate-500">Bubble size represents wait time; color represents risk status</p>
              </div>
              <button
                id="btn-inspect-q6"
                ...
                <ScatterChart margin={{ top: 10, right: 10, bottom: 10, left: -20 }}>
                  ...
                  <Tooltip 
                    ...
                    formatter={(val: any, name: string) => [
                      name === 'Avg Stay (Days)' ? `${val} days` : name === 'Readmission Rate (%)' ? `${val}%` : `${val} min`,
                      name
                    ]}
                  />
                  <Scatter data={scatterData}>
                    {scatterData.map((entry, index) => {
                      const color = entry.status === 'Critical' 
                        ? '#ef4444' 
                        : entry.status === 'High Risk' 
                        ? '#f97316' 
          ...
          <div className="flex items-center justify-center space-x-4 pt-3 border-t border-slate-100 text-xs text-slate-600">
        ...
        <div className="lg:col-span-5 bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="font-bold text-base text-slate-900">Waiting-Time Stratification</h3>
            ...
            <div className="space-y-3">
              {WAIT_TIME_CLASSIFICATION.map((tier) => (
                <div key={tier.category} className="p-3.5 rounded-xl border border-slate-100 bg-slate-50/60 space-y-2">
                  <div className="flex items-center justify-between text-xs font-bold">
                    <span className="text-slate-800">{tier.category}</span>
                    <span className="font-mono px-2 py-0.5 rounded text-white text-[10px]" style={{ backgroundColor: tier.color }}>
          ...
          <div className="p-3 bg-sky-50 rounded-xl border border-sky-100 text-xs text-sky-900 mt-4">
            <div className="font-bold mb-0.5 flex items-center space-x-1">
              ...
              <span>Target Standard: &lt; 45 minutes</span>
            </div>
            <p className="text-[11px] text-sky-800">
              61.3% of all synthetic admissions face wait times above the 45-minute benchmark, concentrated heavily in emergency admissions.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="p-5 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/70">
          <div>
            <h3 className="font-bold text-base text-slate-900">Ranked Department Bottleneck Index</h3>
            <p className="text-xs text-slate-500">Full multi-metric department comparison</p>
          </div>
              ...
              {filteredBottlenecks.map((item) => {
                ...
                return (
                  <tr key={`${item.hospital_name}-${item.department_name}`} className="hover:bg-sky-50/40 transition-colors">
                    ...
                    <td className="px-4 py-3 font-extrabold text-sky-800 font-mono">
